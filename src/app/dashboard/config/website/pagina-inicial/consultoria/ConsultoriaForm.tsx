...
interface ConsultoriaFormProps {
  initialData?: ConsultoriaBackendResponse;
}

export default function ConsultoriaForm({ initialData }: ConsultoriaFormProps) {
  const [content, setContent] = useState<ConsultoriaContent>({
    titulo: "",
    descricao: "",
    buttonUrl: "",
    buttonLabel: "",
  ...
  useEffect(() => {
    const applyData = (first: ConsultoriaBackendResponse) => {
      setContent({
        id: first.id,
        titulo: first.titulo ?? "",
    ...
    const fetchData = async () => {
      setIsFetching(true);
      try {
        const data = await listConsultoria();
        const first = data[0];
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const saved = content.id
        ? await updateConsultoria(content.id, payload)
