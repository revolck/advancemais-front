...
export function CandidatosRow({ candidato }: CandidatosRowProps) {
  ...
  const candidaturasRelevantes: Candidatura[] =
    candidato.candidaturas?.filter((c: Candidatura) =>
      ["RECEBIDA", "EM_ANALISE", "EM_TRIAGEM", "ENTREVISTA"].includes(c.status)
    ) || [];

  const statusInfo = getStatusInfo(
    candidaturasRelevantes?.[0]?.status || "RECEBIDA"
  );
        ...
        <div className="flex items-center justify-center gap-2">
          <div className="flex items-center justify-center w-6 h-6 bg-blue-100 rounded-full">
            <span className="text-xs font-medium text-blue-600">
              {candidaturasRelevantes.length}
            </span>
          </div>
          <span className="text-sm text-gray-900">
            {formatCandidaturasCount(candidaturasRelevantes)}
          </span>
        </div>
      </TableCell>
      ...
      <TableCell className="py-4">
        <div className="flex items-center justify-center gap-2 text-sm text-gray-600">
          ...
          <span className="truncate">
            {candidaturasRelevantes?.[0]?.aplicadaEm
              ? formatDate(candidaturasRelevantes[0].aplicadaEm)
              : "N/A"}
          </span>
        </div>
      </TableCell>
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                aria-label="Avaliar candidato"
              >
                <Eye className="h-4 w-4" />
              </Button>
            </TooltipTrigger>
            <TooltipContent sideOffset={8}>Avaliar candidato</TooltipContent>
          </Tooltip>
