...
export function EmpresaVagasDashboard() {
  ...
  const [pagination, setPagination] = useState<AdminCompanyPagination | null>(
    null
  );
  ...
  const remainingSlots = hasLimitedPlan
    ? Math.max(totalLimit - publishedCount, 0)
    : null;
  const hasReachedLimit = hasLimitedPlan && remainingSlots === 0;

  const planStatus = plan?.status ?? null;
  const hasActivePlan =
    Boolean(plan) && (planStatus ? planStatus === "ATIVO" : true);
  ...
  const fetchVacancies = useCallback(
    async (page = 1) => {
      ...
      try {
        const response = await listAdminCompanyVacancies(tenantId, {
          page,
          pageSize: PAGE_SIZE,
        });
        ...
        if ("data" in response) {
          ...
          const message =
            response?.message ?? "Erro ao carregar vagas da empresa.";
          setVacancies([]);
    ...
    if (!company) {
      return (
        <Alert variant="destructive">
          ...
          <AlertDescription>
            Não conseguimos identificar a empresa vinculada ao seu usuário.
            Configure o tenant e tente novamente.
          </AlertDescription>
        </Alert>
      );
    }
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div className="space-y-1">
            ...
            <p className="text-sm text-slate-600">
              Gerencie as vagas publicadas, em análise ou em andamento da sua
              empresa.
            </p>
          </div>
          <ButtonCustom
    ...
    <div className="space-y-6">
      ...
      {hasReachedLimit && (
        <Alert variant="destructive" className="border-rose-200 bg-rose-50">
          ...
          <AlertDescription>
            Seu plano atual permite publicar até {totalLimit} vagas
            simultaneamente. Para liberar espaço, encerre alguma vaga ativa ou
            atualize seu plano.
          </AlertDescription>
        </Alert>
      )}
          ...
          <TableBody>
            ...
            {!isLoadingVacancies &&
              ...
              vacancies.map((vaga) => {
                ...
                const statusLabel =
                  STATUS_LABELS[statusKey] ?? vaga.status ?? "—";
                const statusTone =
