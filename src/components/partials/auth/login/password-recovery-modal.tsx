...
export function PasswordRecoveryModal({
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (status === 404) {
        ...
        setError(
          "Muitas tentativas. Aguarde alguns instantes para tentar novamente."
        );
      } else {
        setError(
  ...
  const description = isEmailMode
    ? "Digite seu e-mail cadastrado para receber um link seguro e redefinir sua senha."
    : "Informe o CPF ou CNPJ da conta. Vamos localizar seu cadastro e enviar as próximas instruções por e-mail.";
    ...
    >
      <ModalContentWrapper>
        <form onSubmit={handleSubmit} noValidate>
          <ModalHeader>
            <ModalTitle className="!mb-0">Recuperar acesso</ModalTitle>
            <ModalDescription className="!mt-0 !leading-normal !text-sm">
              {description}
            </ModalDescription>
          </ModalHeader>
