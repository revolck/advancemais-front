...
export function CandidaturasTab({
  ...
  const {
    ...
    queryKey: ["candidato-candidaturas", aluno.id],
    queryFn: async () => {
      try {
        console.log("🔍 Buscando candidaturas para aluno:", aluno.id);
        const candidato = await buscarCandidatoPorId(aluno.id);

        if (!candidato) {
          console.warn("⚠️ Candidato não encontrado para ID:", aluno.id);
          return null;
        }
        ...
        console.log("✅ Candidato encontrado:", {
          ...
          candidaturas: candidato.candidaturas?.map((c) => ({
            id: c.id,
  ...
  const candidaturas = candidaturasFiltradas.slice(startIndex, endIndex);
  ...
  if (todasCandidaturas.length === 0) {
    return (
      <EmptyState
        ...
        description="Este aluno ainda não se candidatou a nenhuma vaga."
        maxContentWidth="md"
      />
    );
  }
