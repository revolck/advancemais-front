...
export function CandidaturasTab({
  ...
  // Isso retorna as candidaturas já incluídas na resposta
  const { data: usuarioData, isLoading: isLoadingUsuario } = useQuery({
    queryKey: ["usuario-candidaturas", aluno.id],
    queryFn: async () => {
      try {
        const response = await getUsuarioById(aluno.id);
  ...
  const candidaturas = usuarioData?.candidaturas || [];
