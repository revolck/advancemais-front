...
interface EditarCandidatoEnderecoModalProps {
  isOpen: boolean;
...
export function EditarCandidatoEnderecoModal({
  isOpen,
  onOpenChange,
  candidato,
  onConfirm,
}: EditarCandidatoEnderecoModalProps) {
  const first = candidato.enderecos?.[0];
  const initialValues = useMemo<AddressFormState>(
    () => ({
      cep: first?.cep ? normalizeCep(first.cep) : "",
      cidade: first?.cidade ?? candidato.cidade ?? "",
      estado: first?.estado ?? candidato.estado ?? "",
      bairro: first?.bairro ?? "",
      logradouro: first?.logradouro ?? "",
      numero: first?.numero ?? "",
    }),
    [first, candidato.cidade, candidato.estado]
  );
  ...
  const handleSave = useCallback(async () => {
    if (isSaving) return;
    // validação de CEP (mesmo padrão da modal de empresa)
    ...
    >
      <ModalContentWrapper>
        ...
        <ModalFooter className="pt-4">
          <ButtonCustom variant="outline" onClick={handleClose} size="md" disabled={isSaving}>
            Cancelar
          </ButtonCustom>
          <ButtonCustom
            variant="primary"
