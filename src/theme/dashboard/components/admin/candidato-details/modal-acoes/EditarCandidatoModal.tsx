...
export function EditarCandidatoModal({
  ...
  const initialValues = useMemo(() => {
    ...
    const telefoneMasked = candidato.telefone
      ? maskService.applyMask(candidato.telefone, "phone")
      : "";
    return {
      ...
      dataNasc: candidato.dataNasc ? candidato.dataNasc : "",
      descricao: candidato.descricao ?? "",
      status: candidato.status ?? "",
      instagram: candidato.socialLinks?.instagram ?? "",
      linkedin: candidato.socialLinks?.linkedin ?? "",
    };
  }, [candidato]);
  ...
  const handleSave = useCallback(async () => {
    ...
    const payload: Partial<CandidatoDetailsData> = {};
    ...
    if (!nomeSanitizado) {
      toastCustom.error({
        ...
        description: "Informe o nome completo do candidato.",
      });
      return;
    }
    if (nomeSanitizado !== (candidato.nomeCompleto || "")) {
      payload.nomeCompleto = nomeSanitizado ?? "";
      hasChanges = true;
    }

    if (emailSanitizado !== (candidato.email || "")) {
    ...
    }
    {
      ...
      const currentDigits = (candidato.telefone || "").replace(/\D/g, "");
    ...
    if (nextSocial) {
      (payload as Partial<CandidatoDetailsData>).socialLinks = nextSocial;
      hasChanges = true;
    }
    ...
    if ((generoSanitizado ?? null) !== (candidato.genero ?? null)) {
    ...
    if ((formState.dataNasc || null) !== (candidato.dataNasc || null)) {
    ...
    try {
      ...
      toastCustom.success({
        ...
        description: "As informações do candidato foram salvas com sucesso.",
      });
      handleClose();
    } catch (error) {
  ...
  }, [candidato, formState, handleClose, isSaving, onConfirm]);
  ...
  return (
    ...
    >
      <ModalContentWrapper>
        <ModalHeader>
          <ModalTitle className="!mb-0">Editar candidato</ModalTitle>
        </ModalHeader>
