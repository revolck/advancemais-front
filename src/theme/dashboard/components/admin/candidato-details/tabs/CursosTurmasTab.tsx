...
import type { CursosTurmasTabProps } from "../types";
...
export function CursosTurmasTab({
  ...
  inscricoes = [],
  isLoading,
}: CursosTurmasTabProps) {
  // Buscar dados do usuário usando getUsuarioById (mesma API usada em CurriculosTab)
  // Isso retorna as inscrições já incluídas na resposta
  const { data: usuarioData, isLoading: isLoadingUsuario } = useQuery({
    queryKey: ["usuario-inscricoes", candidato.id],
    queryFn: async () => {
      try {
  ...
  // A API agora retorna os dados no formato correto (UsuarioCursoInscricao)
  const inscricoesData = useMemo<UsuarioCursoInscricao[]>(
    () => (inscricoes && inscricoes.length > 0 ? inscricoes : usuarioData?.cursosInscricoes) || [],
    [inscricoes, usuarioData?.cursosInscricoes]
  );
  const pageSize = 12;
  ...
  if (inscricoesData.length === 0) {
    return (
      <EmptyState
        ...
        description="Este candidato ainda não possui cursos/turmas associados."
