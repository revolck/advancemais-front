...
interface BloquearInstrutorModalProps {
  isOpen: boolean;
  onOpenChange: (open: boolean) => void;
  instrutorNome: string;
  onConfirm: (data: BloquearInstrutorData) => Promise<void>;
}

export interface BloquearInstrutorData {
...
export function BloquearInstrutorModal({
  isOpen,
  onOpenChange,
  instrutorNome,
  onConfirm,
}: BloquearInstrutorModalProps) {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      const payload: BloquearInstrutorData = {
        tipo: state.tipo as
          | "TEMPORARIO"
          | "PERMANENTE"
          | "RESTRICAO_DE_RECURSO",
        motivo: state.motivo as
          | "SPAM"
          | "VIOLACAO_POLITICAS"
          | "FRAUDE"
          | "ABUSO_DE_RECURSOS"
      ...
      toastCustom.success({
        title: "Instrutor bloqueado",
        description:
          "O acesso do instrutor foi bloqueado e o período selecionado está ativo.",
      });

      handleClose();
    } catch (error) {
      console.error("Erro ao bloquear instrutor", error);
      toastCustom.error({
        title: "Erro ao aplicar bloqueio",
        description:
          "Não foi possível bloquear o instrutor agora. Tente novamente em instantes.",
      });
    } finally {
      setIsSubmitting(false);
    }
  ...
  return (
    ...
    >
      <ModalContentWrapper>
        <ModalHeader>
          <ModalTitle>Bloquear instrutor</ModalTitle>
        </ModalHeader>
        ...
        <ModalBody className="space-y-8 p-1">
          ...
          <div className="space-y-3">
            <SimpleTextarea
              ...
              disabled={isSubmitting}
              placeholder="Descreva detalhadamente as observações que justificam o bloqueio de acesso do instrutor..."
