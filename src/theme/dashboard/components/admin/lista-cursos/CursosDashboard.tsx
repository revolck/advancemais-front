...
export function CursosDashboard({
  ...
  useEffect(() => {
    if (!shouldFetch || !queryData?.filters?.applied || filtersInitialized)
      return;
    ...
    if (
      ...
      Array.isArray(applied.statusPadrao) &&
      applied.statusPadrao.length > 0
    ) {
  ...
  const showEmptyState =
    ...
    !isFetchingData &&
    !shouldShowSkeleton &&
    (isEmptyBasedOnMeta || displayedCursos.length === 0);
