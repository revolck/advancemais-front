...
import { useEntrevistasDashboardQuery } from "./hooks/useEntrevistasDashboardQuery";
import type { EntrevistaOverviewItem } from "@/api/entrevistas";
import { AddEntrevistaModal } from "./components/AddEntrevistaModal";
import { EntrevistaModalView } from "./components/EntrevistaModalView";
...
export function EntrevistasDashboard({ className }: { className?: string }) {
  ...
  const normalizedFilters = useMemo(
    ...
    [appliedSearchTerm, currentPage, selectedDateRange.from, selectedDateRange.to, selectedModalidades, selectedStatuses],
  );

  const entrevistasQuery = useEntrevistasDashboardQuery(normalizedFilters);
  ...
  const showEmptyState = !entrevistasQuery.isLoading && !entrevistasQuery.isFetching && entrevistas.length === 0;
      ...
      <div className="overflow-hidden rounded-xl border border-gray-200 bg-white">
        {showEmptyState ? (
          <EmptyState
            ...
            description="Não encontramos entrevistas com os filtros aplicados."
          />
        ) : (
          <>
                  ...
                  {entrevistasQuery.isLoading || entrevistasQuery.isFetching ? (
                    ...
                    entrevistas.map((entrevista) => (
                      <EntrevistaRow
                        ...
                        onViewInterview={setSelectedEntrevista}
                      />
                    ))
                  )}
                </TableBody>
