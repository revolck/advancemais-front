...
interface BloquearUsuarioModalProps {
  isOpen: boolean;
  onOpenChange: (open: boolean) => void;
  usuarioNome: string;
  usuarioRole?: string;
  onConfirm: (data: BloquearUsuarioData) => Promise<void>;
}

export interface BloquearUsuarioData {
...
export function BloquearUsuarioModal({
  isOpen,
  onOpenChange,
  usuarioNome,
  usuarioRole,
  onConfirm,
}: BloquearUsuarioModalProps) {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      const payload: BloquearUsuarioData = {
        tipo: state.tipo as
          | "TEMPORARIO"
          | "PERMANENTE"
      ...
      const roleLabel = usuarioRole ? getRoleLabel(usuarioRole) : "Usuário";
      toastCustom.success({
        title: `${roleLabel} bloqueado`,
        description:
          "O acesso foi bloqueado e o período selecionado está ativo.",
      });
      ...
      toastCustom.error({
        ...
        description:
          "Não foi possível bloquear o usuario agora. Tente novamente em instantes.",
      });
    } finally {
      setIsSubmitting(false);
    }
  ...
  return (
    ...
    >
      <ModalContentWrapper>
        <ModalHeader>
          <ModalTitle>Bloquear usuario</ModalTitle>
        </ModalHeader>
        ...
        <ModalBody className="space-y-8 p-1">
          ...
          <div className="space-y-3">
            <SimpleTextarea
              ...
              disabled={isSubmitting}
              placeholder="Descreva detalhadamente as observações que justificam o bloqueio de acesso do usuario..."
