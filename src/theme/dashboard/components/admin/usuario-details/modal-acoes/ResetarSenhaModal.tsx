...
interface ResetarSenhaUsuarioModalProps {
  isOpen: boolean;
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (method === "email") {
      ...
      try {
        ...
        toastCustom.success({
          ...
          description: "O usuário receberá um e-mail com instruções.",
        });
        handleClose();
      } catch (error) {
    ...
    try {
      ...
      toastCustom.success({
        ...
        description:
          "A senha do usuário foi redefinida. Compartilhe a nova senha com o usuário.",
      });
      handleClose();
    } catch (error) {
        ...
        <ModalBody className="space-y-6 p-1">
          <div className="space-y-4">
            <ModalDescription className="!text-sm !text-muted-foreground">
              Escolha como deseja redefinir a senha da conta do usuário.
            </ModalDescription>
            ...
            >
              ...
              >
                <RadioGroupItem
                  value="email"
                  id="reset-usuario-email"
                  className="mt-1"
                />
                <div className="flex flex-1 flex-col gap-1 text-sm">
                  <Label
                    htmlFor="reset-usuario-email"
                    className="cursor-pointer font-semibold text-foreground"
                  >
                    Enviar instruções por e-mail
                  </Label>
                  <span className="text-xs text-muted-foreground">
                    Dispara um e-mail para o usuário definir uma nova senha.
                  </span>
                </div>
              </div>
              ...
              {allowManual && (
                ...
                >
                  <RadioGroupItem
                    value="manual"
                    id="reset-usuario-manual"
                    className="mt-1"
                  />
                  <div className="flex flex-1 flex-col gap-1 text-sm">
                    <Label
                      htmlFor="reset-usuario-manual"
                      className="cursor-pointer font-semibold text-foreground"
                    >
          ...
          {method === "email" ? (
            ...
            <div className="space-y-4">
              <p className="!text-sm !leading-normal !text-muted-foreground">
                Defina manualmente uma nova senha para a conta do usuário.
              </p>
              <InputCustom
