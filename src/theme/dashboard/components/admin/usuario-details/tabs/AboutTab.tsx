...
export function AboutTab({ usuario, isLoading = false }: AboutTabProps) {
  const aboutDescription = usuario.descricao?.trim();
  ...
  const aboutSidebar: Array<{
    ...
    { label: "Código", value: usuario.codUsuario ?? "—", icon: Tag },
    { label: "Função", value: getRoleLabel(usuario.role), icon: UserCog },
    {
      label: "Criado em",
      value: formatDateTime(usuario.criadoEm),
      icon: CalendarDays,
    },
    {
      label: usuario.tipoUsuario === "PESSOA_JURIDICA" ? "CNPJ" : "CPF",
      value:
    ...
    {
      label: "Telefone",
      value: formatPhoneLink(usuario.telefone || usuario.celular),
      icon: Phone,
    },
    ...
    {
      label: "LinkedIn",
      value: formatSocialLink(usuario.socialLinks?.linkedin),
      icon: Linkedin,
    },
    {
      label: "Localização",
      value: (() => {
        const street = usuario.enderecos?.[0]?.logradouro?.trim();
        const city = usuario.enderecos?.[0]?.cidade?.trim();
        const state = usuario.enderecos?.[0]?.estado?.trim();
        const cep = usuario.enderecos?.[0]?.cep?.trim();
    ...
    {
      label: "Atualizado em",
      value: formatDateTime(usuario.atualizadoEm),
      icon: Clock,
    },
    {
      label: "Último login",
      value: formatDateTime(usuario.ultimoLogin),
      icon: Clock,
    },
  ];
      ...
      <section className="rounded-2xl border border-gray-200/60 bg-white p-6">
        {aboutDescription ? (
          ...
          <EmptyState
            ...
            illustrationAlt="Ilustração de descrição vazia do usuario"
            title="Descrição não adicionada."
            description="Até o momento, este usuario não preencheu uma descrição em seu perfil."
            maxContentWidth="md"
          />
        )}
      </section>
