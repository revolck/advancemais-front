...
interface CursosPopularesListProps {
  cursos: CursoPerformance[];
  isLoading?: boolean;
  maxItems?: number;
}

export function CursosPopularesList({
  cursos,
  isLoading = false,
  maxItems = 5,
}: CursosPopularesListProps) {
  const displayedCursos = cursos?.slice(0, maxItems) || [];
  ...
  if (!displayedCursos || displayedCursos.length === 0) {
    return (
      <div className="bg-white rounded-xl border border-gray-200/60 p-4">
        <div className="flex items-center gap-2 mb-4">
        ...
        <p className="text-xs text-gray-500 text-center py-4">
          Nenhum curso popular encontrado
        </p>
      </div>
    );
  }
        ...
        {displayedCursos.map((curso, index) => (
          <Link
            ...
            className={cn(
              ...
              index !== displayedCursos.length - 1 && "border-b border-gray-200/60 pb-2"
            )}
          >
            {/* Avatar/Icon */}
            <div className="size-8 rounded-full bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center flex-shrink-0">
