...
import {
  createRecrutadorCandidatoEntrevista,
  getRecrutadorCandidatoEntrevistaOpcoes,
  type RecrutadorCandidatoCreateEntrevistaPayload,
  type RecrutadorCandidatoEntrevistaOpcaoItem,
} from "@/api/recrutador";
import type {
  EntrevistaEndereco,
...
interface MarcarEntrevistaRecrutadorModalProps {
  candidatoId: string;
  isOpen: boolean;
...
function buildCompanyDisplayName(item?: RecrutadorCandidatoEntrevistaOpcaoItem | null) {
...
function buildCandidaturaLabel(item: RecrutadorCandidatoEntrevistaOpcaoItem): string {
  return item.vaga?.titulo ?? "Vaga";
}

function validateForm(
  values: FormValues,
  selectedOption: RecrutadorCandidatoEntrevistaOpcaoItem | null,
  capabilities?: EntrevistaOverviewCapabilities,
): FormErrors {
  ...
  if (!values.empresaId) {
    errors.empresaId = "Selecione a empresa.";
  }
  if (!values.candidaturaId) {
    errors.candidaturaId = "Selecione a vaga da entrevista.";
  }
  if (!selectedOption) {
    errors.candidaturaId = errors.candidaturaId ?? "Selecione uma candidatura válida.";
  }
  ...
  if (selectedOption?.entrevistaAtiva) {
    errors.candidaturaId = "Esta candidatura já possui uma entrevista ativa.";
  }
...
export function MarcarEntrevistaRecrutadorModal({
  candidatoId,
  isOpen,
  onOpenChange,
}: MarcarEntrevistaRecrutadorModalProps) {
  ...
  const optionsQuery = useQuery({
    queryKey: ["recrutador-candidato-entrevistas-opcoes", candidatoId],
    queryFn: () => getRecrutadorCandidatoEntrevistaOpcoes(candidatoId),
    enabled: isOpen && Boolean(candidatoId),
    staleTime: 60 * 1000,
  ...
  const creatableOptions = useMemo(
    () => allOptions.filter((item) => !item.entrevistaAtiva),
    [allOptions],
  );
  const canCreate = useMemo(
    () => optionsQuery.data?.canCreate ?? creatableOptions.length > 0,
    [creatableOptions.length, optionsQuery.data?.canCreate],
  );
  const blockedOptionsCount = allOptions.length - creatableOptions.length;

  const companyOptions = useMemo<SelectOption[]>(() => {
    const companies = new Map<string, { label: string; searchKeywords: string[] }>();
  ...
  const filteredOptions = useMemo(
    ...
    [allOptions, formValues.empresaId],
  );

  const selectedOption = useMemo(
    () =>
      allOptions.find((item) => item.candidaturaId === formValues.candidaturaId) ??
      null,
    [allOptions, formValues.candidaturaId],
  );

  const canCreateOnline = capabilities?.canCreateOnline ?? true;
  const canCreatePresencial = capabilities?.canCreatePresencial ?? true;
  const requiresGoogleForOnline = capabilities?.requiresGoogleForOnline ?? false;
  ...
  const candidaturaOptions = useMemo<SelectOption[]>(
    () =>
      filteredOptions.map((item) => ({
        ...
        searchKeywords: [item.vaga?.titulo ?? "", item.vaga?.codigo ?? ""],
        disabled: item.entrevistaAtiva,
      })),
    [filteredOptions],
  );

  const hasOnlyBlockedOptionsInSelectedCompany =
    Boolean(formValues.empresaId) &&
    filteredOptions.length > 0 &&
    filteredOptions.every((item) => item.entrevistaAtiva);

  const applyEnderecoFromOption = (candidaturaId: string | null) => {
    const item =
      allOptions.find((option) => option.candidaturaId === candidaturaId) ?? null;
    return normalizeEnderecoForm(item?.enderecoPadraoEntrevista);
  };

  useEffect(() => {
    if (!isOpen || !optionsQuery.data) return;

    const defaults = optionsQuery.data.defaults;
    const defaultOption =
      creatableOptions.find(
        (item) => item.candidaturaId === defaults?.candidaturaId,
      ) ??
      creatableOptions.find((item) => item.vaga?.id === defaults?.vagaId) ??
      creatableOptions.find(
        (item) => item.empresa?.id === defaults?.empresaUsuarioId,
      ) ??
      creatableOptions[0] ??
      null;

    setFormValues({
      ...buildEmptyForm(optionsQuery.data),
      empresaId: defaultOption?.empresa?.id ?? defaults?.empresaUsuarioId ?? null,
      candidaturaId: defaultOption?.candidaturaId ?? null,
      empresaAnonima: Boolean(defaultOption?.empresaAnonima),
      modalidade: getPreferredModalidade(optionsQuery.data),
      enderecoPresencial: normalizeEnderecoForm(
        defaultOption?.enderecoPadraoEntrevista,
      ),
    });
    setErrors({});
  }, [creatableOptions, isOpen, optionsQuery.data]);
  ...
  useEffect(() => {
    ...
    const fallbackOption =
      filteredOptions.find((item) => !item.entrevistaAtiva) ?? null;
  ...
  }, [filteredOptions, formValues.candidaturaId, formValues.empresaId, formValues.modalidade]);
  ...
  }, [capabilities, canCreateOnline, canCreatePresencial, formValues.modalidade, isOpen]);
  ...
  }, [cidadeOptionsByEstado, formValues.enderecoPresencial.estado, formValues.modalidade]);

  const createMutation = useMutation({
    mutationFn: (payload: RecrutadorCandidatoCreateEntrevistaPayload) =>
      createRecrutadorCandidatoEntrevista(candidatoId, payload),
    onSuccess: async (entrevista) => {
      ...
      await Promise.all([
        queryClient.invalidateQueries({
          queryKey: ["recrutador-candidato-entrevistas", candidatoId],
          exact: false,
        }),
        queryClient.invalidateQueries({
          queryKey: ["recrutador-candidato-entrevistas-opcoes", candidatoId],
      ...
      toastCustom.success({
        ...
        description: "A entrevista foi criada a partir do detalhe do candidato.",
      });
  ...
  const handleCandidaturaChange = (candidaturaId: string | null) => {
    const option =
      allOptions.find((item) => item.candidaturaId === candidaturaId) ?? null;
  ...
  const handleEmpresaChange = (empresaId: string | null) => {
    const fallbackOption =
      allOptions.find(
        (item) => item.empresa?.id === empresaId && !item.entrevistaAtiva,
      ) ?? null;
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (
      ...
      !selectedOption?.empresa?.id ||
      !selectedOption?.vaga?.id
    ) {
      setErrors((current) => ({
        ...current,
    ...
    const payload: RecrutadorCandidatoCreateEntrevistaPayload = {
      candidaturaId: formValues.candidaturaId,
      empresaUsuarioId: selectedOption.empresa.id,
      vagaId: selectedOption.vaga.id,
    ...
    if (formValues.modalidade === "PRESENCIAL") {
      const enderecoPresencial = buildEnderecoPayload(formValues.enderecoPresencial);
    ...
    >
      <ModalContentWrapper className="flex max-h-[90dvh] flex-col overflow-hidden">
        ...
        <ModalBody className="mt-0 flex-1 min-h-0 space-y-6 overflow-y-auto pr-2">
          ...
          {optionsQuery.isError ? (
          ...
          ) : optionsQuery.isSuccess && !canCreate ? (
            <div className="rounded-2xl border border-slate-200/80 bg-white p-8">
              <EmptyState
                illustration="userProfiles"
                illustrationAlt="Nenhuma candidatura elegível"
                title="Nenhuma candidatura elegível"
                description={
                  blockedOptionsCount > 0
                    ? "As candidaturas visíveis deste candidato já possuem entrevista ativa ou não podem receber uma nova entrevista agora."
                    : "Este candidato não possui candidaturas elegíveis para nova entrevista dentro do escopo atual do recrutador."
                }
                maxContentWidth="md"
              />
            </div>
            ...
            <form
              id="create-candidato-entrevista-form"
              className="space-y-6"
            ...
            >
              <div className="space-y-4">
                ...
                {hasOnlyBlockedOptionsInSelectedCompany ? (
                  <Alert>
                    <AlertCircle className="h-4 w-4" />
                    <AlertDescription>
                      Todas as vagas visíveis desta empresa já possuem entrevista ativa.
                    </AlertDescription>
                  </Alert>
                ) : null}
        ...
        <ModalFooter className="border-t border-gray-100 pt-4 sm:flex-row sm:items-center sm:justify-end">
          <div className="flex flex-col gap-2 sm:flex-row">
            ...
            {!optionsQuery.isError && canCreate ? (
              <ButtonCustom
                type="submit"
                form="create-candidato-entrevista-form"
                variant="primary"
